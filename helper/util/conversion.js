...
export const convertToCurrency = (input, isFull = false) => {
  let number = Number(input);
  if (!isNaN(number)) {
    number = isFull ? number : Math.floor(number * 100) / 100; // round down to 2 decimal places

    let [whole, decimal] = isFull ? number.toString().split('.') : number.toFixed(2).toString().split('.');
    if (!decimal) decimal = '';
    whole = whole.replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    return <><InlineSVG src={`<svg id='busd' width="0.7em" height="0.7em" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
